...
func (app *application) Router() *michi.Router {
	...
	r.Route("/", func(sub *michi.Router) {
		...
		sub.HandleFunc("GET vendororders/{id}", app.AuthMiddleware(app.requireVendorPermission(http.HandlerFunc(app.GetVendorOrdersHandler))))
		...
		sub.HandleFunc("DELETE carts/{id}", app.AuthMiddleware(app.AuthorizeUserUpdate(http.HandlerFunc(app.DeleteCartHandler))))
