...
func (app *application) CreateCartItemHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Check if the user has a table and get the table vendor
	table, err := app.Model.TableDB.GetCustomertable(r.Context(), cartID)
	if err != nil {
	...
	if !isAvailable {
		app.handleRetrievalError(w, r, errors.New("item not available in the required quantity"))
		return
	}
	...
	// Check vendor consistency with table vendor
	if table.VendorID != itemVendorID {
		app.errorResponse(w, r, http.StatusBadRequest, "You must buy from the same vendor as your table.")
		return
	}

	// Check vendor consistency with cart vendor
	...
	// Update cart total price based on item price or discount
...
func (app *application) UpdateCartItemHandler(w http.ResponseWriter, r *http.Request) {
	...
	if itemPrice.Discount != 0 {
		priceToUse = itemPrice.Discount
	} else {
		priceToUse = itemPrice.Price
	}

	if difference > 0 {
		// Increasing quantity
		cart.TotalPrice += float64(difference) * priceToUse
